...
/*   Updated: 2019/03/01 23:21:49 by jblack-b         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pixel		burningship(t_mlx *mlx, int x, int y)
{
	...
	{
		tmp = za;
		za = fabs(tmp * tmp) - zb * zb + mlx->data->fractal[mlx->data->nfractal].ca;
		zb = 2 * fabs(tmp * zb) + mlx->data->fractal[mlx->data->nfractal].cb;
		i++;
	}
	c.r = za;
	c.i = zb;
